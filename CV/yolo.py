...
def create_modules(blocks):
    ...
    for index, x in enumerate(blocks[1:]):
        ...
        if x["type"] == "convolutional":
            ...
            maxpool = nn.MaxPool2d(kernel_size=int(x["size"]), stride=int(x["stride"]))
            module.add_module("maxpool_{0}".format(index), maxpool)
        else:
            print("ERROR: UNKNOWN TYPE {}".format(x["type"]))
    ...
    def forward(self, x):
        ...
        for index, module in enumerate(modules):
            ...
            if module_type == "convolutional" or module_type == "upsample":
                ...
                padded_x = F.pad(x, (0,1,0,1), mode="replicate")
                x = self.module_list[index](padded_x)
                ...
                print("ERROR: UNRECOGNIZED MODULE_TYPE {}".format(module_type))
